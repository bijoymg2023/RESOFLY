...
LeptonThread::LeptonThread() : QThread() {
  ...
  //
  typeColormap = 3; // 1:colormap_rainbow  /  2:colormap_grayscale  /
                    // 3:colormap_ironblack(default)
  ...
  //
  spiSpeed = 18 * 1000 * 1000; // SPI bus speed 18MHz (Lowered for stability)
...
void LeptonThread::setLogLevel(uint16_t newLoglevel) { loglevel = newLoglevel; }
  ...
  while (true) {
    ...
    if (typeLepton == 3) {
      if ((segmentNumber < 1) || (4 < segmentNumber)) {
        ...
        if ((n_wrong_segment % 12) == 0) {
          log_message(5, "[WARNING] Got wrong segment number continuously " +
                             std::to_string(n_wrong_segment) + " times");
        }
        continue;
      }
      if (n_wrong_segment != 0) {
        log_message(
            8, "[WARNING] Got wrong segment number continuously " +
    ...
    if ((autoRangeMin == true) || (autoRangeMax == true)) {
      ...
      for (int iSegment = iSegmentStart; iSegment <= iSegmentStop; iSegment++) {
        for (int i = 0; i < FRAME_SIZE_UINT16; i++) {
          ...
          uint16_t value = (shelf[iSegment - 1][i * 2] << 8) +
    ...
    for (int iSegment = iSegmentStart; iSegment <= iSegmentStop; iSegment++) {
      ...
      for (int i = 0; i < FRAME_SIZE_UINT16; i++) {
        ...
        // flip the MSB and LSB at the last second
        valueFrameBuffer =
            (shelf[iSegment - 1][i * 2] << 8) + shelf[iSegment - 1][i * 2 + 1];
        if (valueFrameBuffer == 0) {
          // Why this value is 0?
          n_zero_value_drop_frame++;
          if ((n_zero_value_drop_frame % 12) == 0) {
            log_message(
                5, "[WARNING] Found zero-value. Drop the frame continuously " +
    ...
    if (n_zero_value_drop_frame != 0) {
      log_message(8,
                  "[WARNING] Found zero-value. Drop the frame continuously " +
