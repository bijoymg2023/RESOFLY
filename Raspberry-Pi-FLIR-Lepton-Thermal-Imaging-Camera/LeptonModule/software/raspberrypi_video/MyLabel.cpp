...
//
//   if (!m_logo.isNull()) {
//     QPixmap scaled = m_logo.scaledToHeight(m_logoHeight,
//     Qt::SmoothTransformation); int x = m_logoMargin; int y = height() -
//     scaled.height() - m_logoMargin; p.drawPixmap(x, y, scaled);
//   }
// }
...
void MyLabel::paintEvent(QPaintEvent *event) {
  ...
  // 2) draw thermal overlay (black pixels become transparent if
  // BLACK_BACKGROUND was used)
  if (m_cfg.thermal.enabled && !m_lastImage.isNull()) {
    QImage a = m_lastImage.convertToFormat(QImage::Format_ARGB32);

    // make pure-black transparent (this works only if thermal background is
    // forced to black)
    ...
    if (m_cfg.thermal.smooth <= 0) {
      ...
      QImage down =
          a.scaled(downSz, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
      scaled =
          down.scaled(size(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    }
    p.save();
