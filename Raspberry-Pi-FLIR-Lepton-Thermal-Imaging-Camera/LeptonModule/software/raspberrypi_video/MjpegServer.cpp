...
QByteArray MjpegServer::loadStatic(const QByteArray &urlPath,
...
static QByteArray
httpResponse(const QByteArray &body,
...
void MjpegServer::handleClient(QTcpSocket *s) {
  ...
  QObject::connect(s, &QTcpSocket::readyRead, this, [this, s]() {
    ...
    if (path.startsWith("/mjpeg")) {
      ...
      // stream timer - optimized for Lepton's 9fps output
      auto *t = new QTimer(s);
      t->setInterval(100); // 10 fps to match Lepton camera rate
      QObject::connect(t, &QTimer::timeout, this, [this, s]() {
        ...
        img.convertToFormat(QImage::Format_RGB888)
            .save(&buf, "JPG", 75); // 75% quality for faster streaming
